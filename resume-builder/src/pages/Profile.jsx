...
function Profile() {
  ...
  const onFinish = async (values) => {
    ...
    try {
      console.log(values);
      const retrieve = await axios.post("api/user/update", {
        ...values,
        _id: user._id,
      });
  ...
  useEffect(() => {
    setImage(`${process.env.REACT_APP_BACKEND_URL}/uploads/${user.photo}`);
  }, []);
            ...
            <ul className="nav nav-tabs" id="myTab" role="tablist">
              <li className="nav-item" role="presentation" id="proimage">
                {image && (
                  <img
                    src={
                      `${process.env.REACT_APP_BACKEND_URL}/uploads/${user.photo}` ||
                      image
                    }
                    height={150}
                    width={187}
