...
const EducationSection = ({ user }) => {
  const [editedEducationDescriptions, setEditedEducationDescriptions] =
    useState(user.education.map((edu) => edu.EduDes));
  const [isEditingEducationDescriptions, setIsEditingEducationDescriptions] =
    useState(new Array(user.education.length).fill(false));
  ...
  const handleSaveClick = async (index) => {
    ...
    await setIsEditingEducationDescriptions(
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_BACKEND_URL}/api/user/template2`,
        {
          _id: user1._id,
          education: [
