...
const CertificationSection = ({ user }) => {
  ...
  const [editedCertificationDescriptions, setEditedCertificationDescriptions] =
    useState(user.Certification.map((edu) => edu.Description));
  const [
    isEditingCertificationDescriptions,
    setIsEditingCertificationDescriptions,
  ] = useState(new Array(user.Certification.length).fill(false));
  ...
  const handleSaveClick = async (index) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_BACKEND_URL}/api/user/template5`,
        { Certification: editedCertificationDescriptions, _id: user1._id }
      );
                  ...
                  <textarea
                    ...
                    onChange={(e) => {
                      const newDescriptions = [
                        ...editedCertificationDescriptions,
                      ];
