...
const ExperienceSection = ({ user }) => {
  ...
  const [editedExperienceDescriptions, setEditedExperienceDescriptions] =
    useState(user.Experience.map((exp) => exp.Rolework));
  const [isEditingExperienceDescriptions, setIsEditingExperienceDescriptions] =
    useState(new Array(user.Experience.length).fill(false));
  ...
  const handleSaveClick = async (index) => {
    ...
    setIsEditingExperienceDescriptions(
      new Array(user.Experience.length).fill(false)
    );
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_BACKEND_URL}/api/user/template3`,
        { Experience: editedExperienceDescriptions, _id: user1._id }
      );
