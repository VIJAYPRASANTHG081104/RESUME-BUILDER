      ...
      <div className="container11">
        <div className="profile11">
          <div className="profile_container11">
            <div className="profile_profileImg">
              <img
                src={`${process.env.REACT_APP_BACKEND_URL}/uploads/${user.photo}`}
                alt=""
              />
            </div>
            <div>
              <h1 className="profile_name11">
                <span className="profile_name_firstName11">
                  {user.Firstname} {user.Lastname}{" "}
                </span>
              </h1>
              <p className="profile_title11" style={{ color: "gray" }}>
                {user.Role}
              </p>
        ...
        <div className="group-111">
          <div className="skills11">
            ...
            <ul className="skills_list description11 displaylist11">
              {user.skills.map((skill) => {
                return (
                  <li
                    style={{ color: "white", right: 40, position: "relative" }}
                  >
                    {skill.Technology} with {skill.Rating} Rating{" "}
                  </li>
                );
              })}
            </ul>
          ...
          <div className="edu11">
            ...
            <div className="edu_item11">
              {user.education.map((edu) => {
                return (
                  <div className="displaylist11" style={{ display: "inline" }}>
                    <p className="item_preTitle11">
                      {edu.Qualification} at {edu.Institution} in the year{" "}
                      {edu.YearRange}
                    </p>
                  </div>
                );
          ...
          <div className="interest11">
            <h3 className="title11" style={{ marginTop: "10px" }}>
              Interest
            </h3>
            <div className="interest_items11">
              {user.Interest.map((int) => {
                return (
                  <div displaylist11 style={{ color: "white", fontSize: 17 }}>
                    {int.Interest}
                  </div>
                );
              })}
              ...
              {user.Experience.map((exp) => {
                return (
                  <div className="displaylist11">
                    ...
                    <h4 className="item_title11">
                      {exp.Company} in {exp.Place}
                    </h4>
                    <p className="item_subtitle11">{exp.jobRole}</p>
                    <p className=" description11">{exp.Rolework}</p>
                  </div>
                );
              })}
            </div>
              ...
              {user.Projects.map((pro) => {
                return (
                  <div className="displaylist11">
                    ...
                    <p className=" description11">{pro.Description}</p>
                  </div>
                );
              })}
            </div>
